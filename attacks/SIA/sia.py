...
class Attack(object):
    ...
    """
    def __init__(self, attack, model, epsilon, targeted, random_start, norm, loss_type, mean, std, TI, ks, device=None):
        """
...
class MIFGSM(Attack):
    def __init__(self, model, epsilon=16/255, alpha=1.6/255, epoch=10, decay=1., targeted=False, random_start=False, 
                norm='linfty', loss_type='CE', device=None, attack='MI-FGSM',mean=None, std=None, TI=False, ks=5, **kwargs):
        super().__init__(attack, model, epsilon, targeted, random_start, norm, loss_type, mean, std,  TI, ks, device,**kwargs)
        self.alpha = alpha
        self.epoch = epoch
        self.decay = decay
...
class SIA(MIFGSM):
    def __init__(self, model, epsilon=16/255, alpha=1.6/255, epoch=10, decay=1., num_copies=20, num_block=3, targeted=False, random_start=False, 
                norm='linfty', loss_type='CE', device=None, attack='SIM', mean=None, std=None, TI=False, ks=5, **kwargs):
        super().__init__(model, epsilon, alpha, epoch, decay, targeted, random_start, norm, loss_type, device, attack, mean, std, TI, ks, **kwargs)
    ...
    def logit_loss(self, logits, target_labels):
        """
        Logit loss function for targeted attacks.
        """
        target_labels = target_labels.repeat(self.num_copies)
        target_logits = logits.gather(1, target_labels.view(-1, 1))
    ...
    def get_loss(self, logits, label):
        ...
        Calculate the loss
        """
        # return self.logit_loss(logits, label) if self.targeted else self.loss(logits, label.repeat(self.num_copies))
        loss = self.loss(logits, label.repeat(self.num_copies))
        return loss if self.loss_type == 'logit' else -loss
