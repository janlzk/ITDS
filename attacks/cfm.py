...
class Render3D(object):
    def __init__(self,config_idx=1,count=1):
        ...
        self.source_models=['pack','pillow','book'] # Import source model list
        ...
        kernel = gkern(kernel_size, 3).type(torch.float32)
        gaussian_kernel = torch.stack([kernel, kernel, kernel])
        self.gaussian_kernel = torch.unsqueeze(gaussian_kernel, 1).cuda()
...
def calculate_v(model, x_adv_or_nes, y, eps, number_of_v_samples, beta, target_label, attack_type, number_of_si_scales,
                prob, resize_rate,  loss_fn, config_idx,renderer):
    ...
    for i in range(number_of_v_samples):
        ...
        if 'I' in attack_type:
            ...
            ghat = calculate_si_ghat(model, x_i, number_of_si_scales, attack_type, prob, resize_rate,  loss_fn)
        else:
            if 'D' in attack_type:
                x_i2 = DI(x_i, prob, resize_rate)
                ...
                x_i2 = x_i
            output_x_adv_or_nes = model(trf_nor(x_i2))
...
# Admix method
def calculate_admix(model, x_adv_or_nes, number_of_si_scales, attack_type, prob, resize_rate,  loss_fn, config_idx, renderer, logit):
    ...
    for _ in range(number_of_admix_samples):
        ...
        if 'S' in attack_type: 
            ...
            if 'D' in attack_type:
                ...
                x_i2 = x_i
            output_x_adv_or_nes = model(trf_nor(x_i2))
            if logit in ['Topkdis', 'Tgkdis']:
                loss = loss_fn(x_i, output_x_adv_or_nes) 
            else:
                loss = loss_fn(output_x_adv_or_nes)  
...
def calculate_si_ghat(model, x_adv_or_nes, number_of_si_scales, attack_type, prob, resize_rate,  loss_fn,renderer):
    x_neighbor = x_adv_or_nes.clone().detach()
    ...
    for si_counter in range(0, number_of_si_scales):
        ...
        if 'D' in attack_type:
            si_input2 = DI(si_input,prob,resize_rate)
        elif 'R' in attack_type:
        ...
        output_si = model(trf_nor(si_input2))
...
def DI(x,prob=0.7,resize_rate=0.9): # DI method
    img_size = x.shape[-1]
...
class FeatureMixup(nn.Module):
    def __init__(self, model: nn.Module, config_idx, input_size, mix_prob, nor):
        super().__init__()
        exp_settings=exp_configuration[config_idx]
    ...
    def save_outputs_hook(self, layer_idx, config_idx) -> Callable:
        ...
        def hook_fn(module, input, output):
            if every_layer:
                if type(module)==torch.nn.Linear or output.size()[-1]<=self.input_size:
...
def advanced_fgsm(attack_type, source_model, x, y, target_label=None,
                  num_iter=100, max_epsilon=16, step_size=1.6, di_prob=1, resize_rate=0.9, nor=2,
                  number_of_v_samples=5, beta=1.5, number_of_si_scales=5, kernel_size=5, mix_prob=0.1, count=0, config_idx=1):
    """
    Args:
    ...
    exp_settings = exp_configuration[config_idx]
    mu = exp_settings['mu']
    beta = exp_settings['beta']
    fix_mode = exp_settings['fix_mode_input']
    zeta = exp_settings['zeta']
    logit = exp_settings['Logit']
    weight = exp_settings['weight']
    top_k = exp_settings['top_k']

    if 'O' in attack_type or 'I' in attack_type or 'V' in attack_type or 'S' in attack_type:
        renderer=Render3D(config_idx=config_idx,count=count)

    step_size=exp_settings['lr'] # Step size
    ...
    if 'T' in attack_type: # Tranlation-invariance
        kernel = gkern(kernel_size, 3).type(torch.float32)
        gaussian_kernel = torch.stack([kernel, kernel, kernel])
        gaussian_kernel = torch.unsqueeze(gaussian_kernel, 1).cuda()
    source_model.eval()
    ...
    if exp_settings['Logit']=='LogitLoss':
        ...
        loss_fn=TopkLoss(target_label, exp_settings['targeted'], top_k, exp_settings['p'])
    elif exp_settings['Logit']=='Topkdis':
        loss_fn=Topkdis(target_label, exp_settings['targeted'], top_k, exp_settings['p'])
    ...
    if 'C' in attack_type: # Storing clean features at the first iteration
        with torch.no_grad():
            ...
            model=FeatureMixup(source_model,config_idx,img_width,mix_prob,nor) # Attach CFM modules to conv and fc layers
            model.start_feature_record() # Set feature recoding mode
            model(trf_nor(x_f)) # Feature recording
            # model(x_f)
    ...
    for t in range(num_iter):
        if t>=consumed_iteration:
            ...
            if 'I' in attack_type: # Admix
                ghat = calculate_admix(model, x_nes, number_of_si_scales, attack_type, di_prob, resize_rate,  loss_fn, config_idx, renderer, exp_settings['Logit']) 
                
            elif 'S' in attack_type:  # Scale-Invariance
                ghat = calculate_si_ghat(model, x_nes, number_of_si_scales, attack_type,
                                            di_prob, resize_rate,  loss_fn, renderer)
            else:
                if 'D' in attack_type:
                    x_adv_or_nes = DI(x_nes,di_prob,resize_rate) 
                elif 'R' in attack_type and 'O' in attack_type:
                    x_adv_or_nes = RDI(render_3d_aug_input(x_nes,renderer=renderer,prob=di_prob))
                elif 'R' in attack_type:
                    x_adv_or_nes = RDI(x_nes)
                elif 'O' in attack_type:
                    x_adv_or_nes = render_3d_aug_input(x_nes,renderer=renderer,prob=di_prob)
                
                else:    
                    x_adv_or_nes = x_nes

                if logit in ['Angle', 'oriAngle']:
                    inp_output2, output2 = model(trf_nor(x_adv_or_nes))
                    loss = loss_fn(inp_output2)
                elif logit in ['Topkdis', 'Tgkdis']:
                    output2 = model(trf_nor(x_adv_or_nes))
                    loss = loss_fn(x_nes, output2) 
                else:
                    output2 = model(trf_nor(x_adv_or_nes))              
                    loss = loss_fn(output2)  


                ghat = torch.autograd.grad(loss, x_nes, retain_graph=False, create_graph=False)[0]

            # Update g
            grad_plus_v = ghat + v  
            if 'T' in attack_type:  # Translation-invariance
                grad_plus_v = F.conv2d(grad_plus_v, gaussian_kernel, bias=None, stride=1, padding=((kernel_size-1)//2,(kernel_size-1)//2), groups=3) #TI
            ...
            if 'V' in attack_type:
                v = calculate_v(model, x_nes, y, eps, number_of_v_samples, beta, target_label, attack_type,
                                number_of_si_scales, di_prob, resize_rate,  loss_fn, config_idx, renderer) - ghat
