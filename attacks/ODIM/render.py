...
class Render3D(object):
    def __init__(self,config_idx=1,count=1):
        ...
        self.count=count
        self.eval_count=0
        exp_settings=exp_configuration[self.config_idx] # Load experiment configuration
        ...
        self.model_settings={
            'pack':['pack.obj',255,255,510,510,1.2,0,0,[0,0.02,0.]],
        ...
        self.background_img=torch.zeros((1,3,224,224)).to(device)
...
def load_object(obj_file_name):
    obj_filename = os.path.join("attacks/ODIM/data", obj_file_name)
